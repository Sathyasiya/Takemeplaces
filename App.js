  ...
  return (
    <nav className="nav">
      ...
      <div className="nav-links">
        ...
        <button className="nav-link" onClick={() => setPage("Login")} style={{border:"1px solid rgba(116,198,157,0.3)", marginLeft:8}}>Login</button>
...
function LoginPage({ setPage }) {
  ...
  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  🔧 FORMSPREE LOGIN SETUP - ADD YOUR FORM ENDPOINT HERE                  ║
  // ╠══════════════════════════════════════════════════════════════════════════╣
  // ║  This captures user registrations and sends them to your email.          ║
  // ║                                                                           ║
  // ║  1. Go to: https://formspree.io/                                         ║
  // ║  2. Create a SECOND form called "User Registrations"                     ║
  // ║  3. Copy your form endpoint URL                                          ║
  // ║  4. Replace YOUR_LOGIN_FORM_ID below with your actual form ID            ║
  // ║                                                                           ║
  // ║  Example:                                                                ║
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // For signup, send registration data to Formspree
      if (!isLogin) {
        const response = await fetch(FORMSPREE_LOGIN, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        // For login (since there's no backend), just simulate success
        setSuccess(true);
        setTimeout(() => {
          setPage("My Bookings");
        }, 1500);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #1a3d2b 0%, #0a7c6e 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
      {/* Background decoration */}
      <div style={{ position: "absolute", top: -100, right: -100, width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(116,198,157,0.1) 0%, transparent 70%)" }} />
      <div style={{ position: "absolute", bottom: -150, left: -150, width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(82,183,136,0.08) 0%, transparent 70%)" }} />
      ...
      <div style={{ position: "relative", zIndex: 2, width: "100%", maxWidth: 440 }}>
        ...
        {/* Login/Signup Card */}
        <div style={{ background: "white", borderRadius: 24, padding: "48px 40px", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
          {/* Logo */}
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{ fontFamily: "Playfair Display", fontSize: "1.8rem", fontWeight: 900, color: "#1a3d2b", marginBottom: 8 }}>
              🌿 Take Me <span style={{ color: "#e9c46a" }}>Places</span>
            </div>
            <p style={{ color: "#6b7c73", fontSize: "0.9rem" }}>
          ...
          {success ? (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              <div style={{ fontSize: "4rem", marginBottom: 20 }}>✅</div>
              <h3 style={{ fontFamily: "Playfair Display", fontSize: "1.6rem", color: "#1a3d2b", marginBottom: 12 }}>
                {isLogin ? "Login Successful!" : "Account Created!"}
              </h3>
              <p style={{ color: "#6b7c73" }}>Redirecting you now...</p>
            </div>
          ) : (
            <>
              {/* Tab Toggle */}
              <div style={{ display: "flex", background: "#f0f7f4", borderRadius: 12, padding: 4, marginBottom: 32 }}>
                <button
                  onClick={() => setIsLogin(true)}
                  style={{
                    flex: 1, padding: "10px", borderRadius: 8, border: "none",
                    background: isLogin ? "white" : "transparent",
                    color: isLogin ? "#1a3d2b" : "#6b7c73",
              ...
              <form onSubmit={handleSubmit}>
                ...
                {isLogin && (
                  <div style={{ textAlign: "right", marginBottom: 24 }}>
                    <button
                      type="button"
                      onClick={() => alert("Please contact support: hello@takemeplaces.in")}
                      style={{ background: "none", border: "none", color: "#40916c", fontSize: "0.85rem", cursor: "pointer", fontFamily: "DM Sans", fontWeight: 500 }}
                    >
                      Forgot password?
                    </button>
                  </div>
                ...
                <button
                  ...
                  style={{ width: "100%", padding: "16px", marginTop: !isLogin ? 24 : 0 }}
                  disabled={submitting}
                >
              ...
              <div style={{ marginTop: 32, textAlign: "center" }}>
                <p style={{ fontSize: "0.85rem", color: "#6b7c73", marginBottom: 16 }}>Or continue with</p>
                  ...
                  {["Google", "Facebook"].map((provider) => (
                    <button
                      ...
                      style={{
                        ...
                        border: "2px solid #e0ede8", background: "white",
                        color: "#6b7c73", fontFamily: "DM Sans",
                        fontSize: "0.9rem", fontWeight: 600, cursor: "pointer",
                        transition: "all 0.2s",
                      }}
                      onMouseOver={(e) => { e.target.style.borderColor = "#40916c"; }}
                      onMouseOut={(e) => { e.target.style.borderColor = "#e0ede8"; }}
                    >
                      {provider}
                    </button>
                  ))}
        ...
        {/* Terms */}
        <p style={{ textAlign: "center", marginTop: 20, fontSize: "0.75rem", color: "rgba(255,255,255,0.6)" }}>
